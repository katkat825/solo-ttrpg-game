namespace Core.Space
{
    ...
    // made for the same reason: what a wall DOES - block a move, block a line - is a rule, and
    // rules are code (CONVENTIONS.md, "rules in code, world in data"). The map that says which
    // squares are walls is content and lives in a data file.
    //
    ...
    // it. Five kinds is not a taxonomy worth paying for yet.
    public enum Tile
    {
        // not part of this map at all - what a cell off the edge of the layout is. impassable and
        // opaque, so pathfinding and sight need no bounds check of their own, and the answer stays
        // honest rather than a wall being invented where the map simply stops
        Void = 0,
        ...
        // shut. it blocks movement and sight exactly like a wall, which is what makes it worth
        // having: B4 opens one on a passed check, and a door that never did anything would be a
        // wall with a different mesh
        Door,
    ...
    public static class Tiles
    {
        ...
        // flag now would have to be unfolded the first time either shows up
